...
// Undertone patch: the Command Line Tools toolchain has no SwiftUI macro plugin, so `@Entry` is spelled out.
private nonisolated struct KeyboardShortcutsConflictPolicyKey: EnvironmentKey {
	// `ConflictPolicy.default` is main-actor isolated by this module's default isolation; SwiftUI reads environment defaults on the main thread.
	static var defaultValue: KeyboardShortcuts.ConflictPolicy {
		MainActor.assumeIsolated { KeyboardShortcuts.ConflictPolicy.default }
	}
}
